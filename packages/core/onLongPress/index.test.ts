...
describe("onLongPress", () => {
  ...
  async function stopPropagation(isRef: boolean) {
    ...
    parentElement.value.addEventListener(
      "pointerdown",
      onParentLongPressCallback
    );
  ...
  function suites(isRef: boolean) {
    ...
    describe("given options", () => {
      it("should trigger longpress after options.delay ms", () =>
        triggerCallbackWithDelay(isRef));
      it("should not tirgger longpress when child element on longpress", () =>
        notTriggerCallbackOnChildLongPress(isRef));
      it("should work with once and prevent modifiers", () =>
        workOnceAndPreventModifiers(isRef));
